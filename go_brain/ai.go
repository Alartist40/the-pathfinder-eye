...
func (a *AIBrain) Process(userInput string, worldContext string) (string, error) {
	...
	return a.processText(userInput, worldContext, "http://localhost:8081/v1/chat/completions", "", "qwen")
}

func (a *AIBrain) processText(userInput, worldContext, url, apiKey, model string) (string, error) {
	systemPrompt := `You are THE-PATHFINDER-EYE, a Pathfinder robot.
You have these tools: move, look, light, play_resource, read_document, activate, deactivate.
Always use a tool when the user asks you to do something physical.
Respond in 1-2 short sentences. Never apologize.
...
func (a *AIBrain) ProcessVision(userInput, worldContext, url, apiKey, model string) (string, error) {
	...
	systemPrompt := `You are THE-PATHFINDER-EYE, a Pathfinder robot.
Analyze image and act using available tools.
Respond in 1-2 short sentences. Never apologize.
WORLD STATE: ` + worldContext
...
func SwapLeafcutterModel(modelPath string) error {
	...
	if err := os.WriteFile(dropInDir+"/override.conf", []byte(content), 0644); err != nil {
		return err
	}
