...
func main() {
	...
	http.HandleFunc("/stream", authWrap(func(w http.ResponseWriter, r *http.Request) {
		data, err := os.ReadFile("/tmp/vision_feed.jpg")
...
func handleCamera(w http.ResponseWriter, r *http.Request) {
	...
	if axis == "pan" {
		...
		if calibration.Pan.Current < calibration.Pan.Min {
			calibration.Pan.Current = calibration.Pan.Min
		}
		if calibration.Pan.Current > calibration.Pan.Max {
			calibration.Pan.Current = calibration.Pan.Max
		}
		_ = setServo(1, byte(calibration.Pan.Current))
	} else {
		calibration.Tilt.Current -= val
		if calibration.Tilt.Current < calibration.Tilt.Min {
			calibration.Tilt.Current = calibration.Tilt.Min
		}
		if calibration.Tilt.Current > calibration.Tilt.Max {
			calibration.Tilt.Current = calibration.Tilt.Max
		}
