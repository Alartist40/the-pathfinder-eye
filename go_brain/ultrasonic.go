...
func readDistanceMM() (int, error) {
	...
	// Read high byte (0x1B)
	bufHigh := make([]byte, 1)
	if _, err := i2cFile.Write([]byte{0x1B}); err != nil {
		return 0, err
	}
	if _, err := i2cFile.Read(bufHigh); err != nil {
		return 0, err
	}

	// Read low byte (0x1A)
	bufLow := make([]byte, 1)
	if _, err := i2cFile.Write([]byte{0x1A}); err != nil {
		return 0, err
	}
	if _, err := i2cFile.Read(bufLow); err != nil {
		return 0, err
	}
