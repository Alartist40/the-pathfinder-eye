...
// MinFreeMBForDeepThought is the minimum MB of free memory required to attempt
// loading the 4B reasoning model. Qwen3.5-4B-Q4_K_M requires about
// 2.7 GB RAM. We set a 2500 MB floor.
const MinFreeMBForDeepThought = 2500
...
func processDirectCommand(cmd string, level AuthorityLevel, name string) bool {
	...
	// Tier 2: Fast rule-based command parser
	cmdParsed := ExtractCommand(cmd)
	if dispatchAction(cmdParsed, level, name) {
		infoLog.Printf("RULE_PARSER_ROUTE: action=%q target=%q raw=%q", cmdParsed.Action, cmdParsed.Target, cmd)
		return true
	}
...
func handleReadAction(cmd ParsedCommand) bool {
	...
	default:
		...
		file = "Pathfinder " + titleCase(cmd.Target) + ".md"
	}
...
func enterDeepThought() {
	// RAM guard — refuse to load the 4B model if we don't have room.
	// Qwen3.5-4B-Q4_K_M is ~2.7 GB on disk/RAM.
	if _, memFreeMB, _ := getMemInfo(); memFreeMB < MinFreeMBForDeepThought {
		infoLog.Printf("DEEP_THOUGHT: refused — only %d MB free, need ≥ %d MB",
			memFreeMB, MinFreeMBForDeepThought)
		if ttsEngine != nil {
	...
	const deepModel = "/home/pi/the-pathfinder-eye_ai/models/Qwen3.5-4B-Q4_K_M.gguf"
