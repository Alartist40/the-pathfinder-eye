...
impl YOLODetector {
    ...
    pub fn detect(&mut self, frame: &Mat) -> Result<Vec<Detection>> {
        ...
        // Check if output is YOLOv8 format: [1, 84, 8400]
        if total == 84 * 8400 || (total % 8400 == 0 && total / 8400 == 84) {
            ...
            for i in 0..num_anchors {
                ...
                for c in 0..80 {
                    let score = slice[(4 + c) * num_anchors + i];
                    if score > max_score {
                        max_score = score;
                        class_id = c as i32;
                    }
                ...
                if max_score < YOLO_CONFIDENCE_THRESHOLD || class_id < 0 {
                    continue;
                }
                ...
                let x = ((cx - w / 2.0) * scale_x).max(0.0) as i32;
                let y = ((cy - h / 2.0) * scale_y).max(0.0) as i32;
                let bw = (w * scale_x) as i32;
                let bh = (h * scale_y) as i32;

                candidates.push((Rect::new(x, y, bw, bh), max_score, class_id));
            }
        } else {
            // YOLOv5 format: [rows, 85]
